...
if ($num > 0) {
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        ...
        echo "<img src='" . $row['proFoto'] . "' alt='" . $row['proNome'] . "' width='150'>";
        echo "<h3>" . $row['proNome'] . "</h3>";
        echo "<p>" . $row['proDescricao'] . "</p>";
        echo "<p>R$ " . number_format($row['proPreco'], 2, ',', '.') . "</p>";
        $lojNome = $produto->getLoja($row['proLojaID']);
        echo "<p>Vendido por <a href='loja.php?lojID=" . $row['proLojaID'] . "'>" . $lojNome . "</a></p>";
        echo "<a href='produto.php?proID=" . $row['proID'] . "'>Ver produto</a>";
        echo "</div>";
    }
} else {
